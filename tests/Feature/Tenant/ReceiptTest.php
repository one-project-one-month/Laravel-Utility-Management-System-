...
describe('Tenant', function(){
    test('get_latest_receipt', function(){
        $room = roomCreate();
        ...
        ->getJson("$this->tenantApi/$tenant->id/receipts/latest")
        ->assertJsonStructure([
                'success',
                'message',
                'content',
    ...
    test('get_receipts_history', function(){
        $room = roomCreate();
        $tenant = tenantCreate($room);
        $userTenant = tenantUserCreate($tenant);
        $bill = billCreate($room,$tenant);
        ...
        $this
        ->actingAs($userTenant, 'sanctum')
        ->getJson("$this->tenantApi/$tenant->id/receipts/history")
        ->assertJsonStructure([
                'success',
                'message',
                'content' => [
    ...
    test('get_receipts_history_returns_empty_list_for_new_tenant', function(){
        $room = roomCreate();
        $tenant = tenantCreate($room);
        $userTenant = tenantUserCreate($tenant);
    ...
    test('user_cannot_get_another_tenants_latest_receipt' ,function(){
        $room1 = roomCreate();
        $room2 = roomCreate();
        ...
        ->getJson("$this->tenantApi/$tenant2->id/receipts/latest")
        ->assertJsonStructure([
            'message',
        ])
        ->assertStatus(401);
